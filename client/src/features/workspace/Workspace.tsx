...
type TabId = 'overview' | 'reader' | 'ideas' | 'paper' | 'virtual' | 'analytics'
...
export function Workspace() {
  ...
  return (
    <div className="min-h-screen">
      <header className="bg-card border-faint border-b">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 text-[21px] font-medium tracking-[-0.8px]">
              ...
              <span className="eqr-logo-spin inline-flex">
                <BotMessageSquare className="size-[22px] text-[var(--up)]" strokeWidth={1.75} />
              </span>
              EQR
            </div>
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center">
                ...
                <IconToggle onClick={save} title="Düzeni kaydet" className="rounded-l-none">
                  {saved ? (
              ...
              <IconToggle
                ...
                title={isCompact ? 'Satır aralığını genişlet' : 'Satır aralığını sıklaştır'}
              >
                {isCompact ? (
              ...
              <IconToggle onClick={toggle} title={isDark ? 'Açık tema' : 'Koyu tema'}>
                {isDark ? <Sun className="size-[15px]" /> : <Moon className="size-[15px]" />}
              </IconToggle>
            </div>
          </div>

          <nav className="mt-[18px] flex items-center gap-6 overflow-x-auto">
            {TABS.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                ...
                style={{
                  borderBottomColor: tab === t.id ? 'var(--info)' : 'transparent',
